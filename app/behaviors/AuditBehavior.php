...
class AuditBehavior extends CActiveRecordBehavior {
		...
		if (!$owner->isNewRecord) {
			foreach ($owner->getAttributes() as $name => $value) {
				...
				if ((string)$value !== (string)$this->_oldAttributes[$name]) {
					...
					$entry->created = $this->sqlDatetime();
					$entry->save();
				}
			}
		}
    ...
    /**
     * Applies the audit criteria to get given criteria.
     * @param CDbCriteria $criteria the criteria.
     * @return CDbCriteria the criteria.
     */
    public function applyAuditCriteria($criteria) {
        $criteria->join = 'LEFT JOIN audit_model ON (t.id=modelId AND model=:model)';
        $criteria->params[':model'] = get_class($this->owner);
        return $criteria;
    }

    /**
     * Returns the given time as a SQL datetime.
     * @param integer $time the time.
     * @return string the date.
     */
    public function sqlDatetime($time = null) {
        return date('Y-m-d H:i:s', $time !== null ? $time : null);
    }
